...
class ResizeShortestEdge(Augmentation):
    ...
    @torch.jit.unused
    def __init__(
        self, short_edge_length, max_size=sys.maxsize, sample_style="range", interp=Image.BILINEAR,fixed_seed=None
    ):
        """
        Args:
    ...
    @torch.jit.unused
    def get_transform(self, image):
        if self.fixed_seed is not None and not hasattr(self, '_fixed_seed_set'):
            np.random.seed(self.fixed_seed)
            self._fixed_seed_set = True
        h, w = image.shape[:2]
        if self.is_range:
...
class FixedSizeCrop(Augmentation):
    ...
    def adjust_size_to_be_divisible_by(self, image, divisor):
        """
        Adjusts the size of the image so that its height and width are divisible by the given divisor.

        Args:
            image (np.ndarray): The image to adjust.
            divisor (int): The divisor that the image's height and width should be divisible by.
        ...
        w_adj = (w // divisor) * divisor
        if h_adj == h and w_adj == w:
            return image
        return image[:h_adj, :w_adj]
