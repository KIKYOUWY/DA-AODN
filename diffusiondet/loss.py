...
from DA_AODN import Target_Contrast_Enhancement_Detection_Loss
class SetCriterionDynamicK(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=False):
        ...
        if self.use_focal or self.use_fed_loss:
            ...
            center_loss_module = Target_Contrast_Enhancement_Detection_Loss(num_classes=self.num_classes, feat_dim=src_logits.shape[-1],
                                            device=src_logits.device, alpha=1, beta=1)
            sigmoid = nn.Sigmoid()
            center_loss = 0.01 * center_loss_module(sigmoid(src_logits), target_classes_onehot)

            losses = {'loss_ce': loss_ce, 'loss_center': center_loss}
