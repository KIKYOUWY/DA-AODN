...
from detectron2.modeling.backbone.MYbackbone import build_clear_dataset,get_next_clear_image
import torch
from DA_AODN import Unsupervised_Clear_Channel_Guided_Loss
...
datasets_name_clear = ["coco_clear"]
clear_dataset_loader = build_clear_dataset(datasets_name_clear)
clear_images_generator = get_next_clear_image(clear_dataset_loader)
...
class DiffusionDet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.UCCGL = Unsupervised_Clear_Channel_Guided_Loss()
        # Build Criterion.
    ...
    def ddim_sample(self, batched_inputs, backbone_feats, images_whwh, images, clip_denoised=True, do_postprocess=True):
        ...
        for time, time_next in time_pairs:
            ...
            if self.box_renewal:  # filter
                # replenish with randn boxesoutputs_class, outputs_coord = self.head(features, x_boxes, t, None)outputs_class, outputs_coord = self.head(features, x_boxes, t, None)outputs_class, outputs_coord = self.head(features, x_boxes, t, None)outputs_class, outputs_coord = self.head(features, x_boxes, t, None)outputs_class, outputs_coord = self.head(features, x_boxes, t, None)outputs_class, outputs_coord = self.head(features, x_boxes, t, None)
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        clear_image = next(clear_images_generator)
        clear_image = clear_image.unsqueeze(0)
        clear_image = self.normalizer(clear_image)
        ...
        src, Guided_image = self.backbone(images.tensor)
        ...
        if self.training:
            ...
            UCCGL = self.UCCGL(clear_image, Guided_image)


            LOSS = 'uccg_loss'

            loss_dict[LOSS] = UCCGL
    ...
    def preprocess_image(self, batched_inputs):
        ...
        # print(self.normalizer(batched_inputs[0]["image"].to(self.device)).dtype)
