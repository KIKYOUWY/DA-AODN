...
from detectron2.modeling.backbone.MYbackbone import build_train_aug
from detectron2.data.dataset_mapper import DatasetMapper
class Trainer(DefaultTrainer):
    """ Extension of the Trainer class adapted to DiffusionDet. """#train的扩展
    ...
    def __init__(self, cfg):
        ...
        data_loader = self.build_train_loader(cfg,1)
        # data_loader_clear = self.build_train_loader(cfg,2)
    ...
    @classmethod
    def build_train_loader(cls, cfg, dataset_type):
        mapper = DatasetMapper(is_train=True, image_format="RGB", augmentations=build_train_aug((512),1333,"choice",42))

        datasets_name_hazy = ["coco_2017_train"]
        dataset_hazy = get_detection_dataset_dicts(datasets_name_hazy)
        sampler = TrainingSampler(len(dataset_hazy),shuffle=True,seed=40244023)
        if dataset_type == 1:
            return build_detection_train_loader(cfg, dataset=dataset_hazy, mapper=mapper,sampler=sampler)
        elif dataset_type == 2:
            return build_detection_train_loader(cfg, dataset=dataset_clear, mapper=mapper,sampler=sampler)
    ...
    def build_hooks(self):
        ...
        cfg = self.cfg.clone()
        cfg.defrost()
        cfg.DATALOADER.NUM_WORKERS = 0  # save some memory and time for PreciseBN多核数据集处理
...
def setup(args):
    ...
    cfg.merge_from_file('configs/CD-AODN.yaml')
